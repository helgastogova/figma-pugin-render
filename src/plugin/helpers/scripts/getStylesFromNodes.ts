      ...
      if (node.type === 'TEXT') {
        ...
        if (uniqueTextColorStyles.length != 0) {
          uniqueTextColorStyles.forEach((fillStyle) => {
            const id = fillStyle.fillStyleId as string
            const style = figma.getStyleById(id) as PaintStyle
        ...
        if (hasFillStyles(node)) {
          if (node.fillStyleId != '') {
            //get the style
            const id = node.fillStyleId as string
            const style = figma.getStyleById(id) as PaintStyle
        ...
        if (hasStrokeStyle(node)) {
          if (node.strokeStyleId != '') {
            //get the style
            const id = node.strokeStyleId as string
            const style = figma.getStyleById(id) as PaintStyle
      ...
      if (node.textStyleId != '') {
        if (typeof node.textStyleId === 'symbol') {
          ...
          uniqueTextStyles.forEach((textStyle) => {
            const id = textStyle.textStyleId as string
            const style = figma.getStyleById(id) as TextStyle

            //add style to the array
            styles.push(style)
          })
        } else {
          const id = node.textStyleId as string
          const style = figma.getStyleById(id) as TextStyle
  ...
  if (styleTypes.effect) {
    ...
    if (hasEffects(node)) {
      if (node.effectStyleId != '') {
        ...
        const style = figma.getStyleById(id) as EffectStyle
