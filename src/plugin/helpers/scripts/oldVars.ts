...
function createTheme(data) {
  ...
  switch (data.source) {
    ...
    case 'page':
      ...
      const pageNodes = Array.from(figma.currentPage.children)
      if (pageNodes) {
        //get styles
        if (data.colorStyles) {
...
function collectColorStyles(node) {
  ...
  if (node.type === 'COMPONENT' || 'INSTANCE' || 'FRAME' || 'GROUP') {
    if (node.backgroundStyleId) {
      const objectStyle = figma.getStyleById(node.backgroundStyleId)
  ...
  if (
    ...
    if (node.fillStyleId) {
      const objectStyle = figma.getStyleById(node.fillStyleId)
      // key will only be available for remote styles
      if (objectStyle.key) {
        const style = {
          name: styleName(objectStyle.name),
    ...
    if (node.strokeStyleId) {
      const objectStyle = figma.getStyleById(node.strokeStyleId)
      // key will only be available for remote styles
      if (objectStyle.key) {
        const style = {
          name: styleName(objectStyle.name),
...
function collectTextStyles(node) {
  ...
  if (node.type === 'TEXT' && node.textStyleId != 'MIXED' && node.textStyleId) {
    const objectStyle = figma.getStyleById(node.textStyleId)
    // key will only be available for remote styles
    if (objectStyle.key) {
      const style = {
        name: styleName(objectStyle.name),
...
function collectEffectStyles(node) {
  ...
  if (node.effectStyleId) {
    const objectStyle = figma.getStyleById(node.effectStyleId)
    // key will only be available for remote styles
    if (objectStyle.key) {
      const style = {
        name: styleName(objectStyle.name),
...
function styleName(name) {
  if (usePrefixes) {
    if (name.includes('/')) {
      const styleName = name.split('/').slice(1).join('.')
      return styleName
    } else {
      figma.notify('Styles names must be prefixed. Ex: themeName/colorName')
    }
...
function countNewThemes() {
  const themes = [...new Set(cleanedStyleData.map((style) => style.theme))]
...
function applyTheme(applyTo) {
  ...
  if (nodes) {
    ...
    const colorStyles = [
      ...new Set(jsonBinData.map((style) => style.theme === selectedTheme && style.type === 'PAINT')),
    ]
    const textStyles = [...new Set(jsonBinData.map((style) => style.theme === selectedTheme && style.type === 'TEXT'))]
    const effectStyles = [
      ...new Set(jsonBinData.map((style) => style.theme === selectedTheme && style.type === 'EFFECT')),
    ]
  ...
  if (node.type === 'COMPONENT' || 'INSTANCE' || 'FRAME' || 'GROUP') {
    if (node.backgroundStyleId) {
      ;(async function () {
        ...
        if (style.key) {
          const newStyleKey = findMatchInSelectedTheme(style.key)
          if (newStyleKey) {
            const newStyle = (await figma.importStyleByKeyAsync(newStyleKey)) as PaintStyle
            if (newStyle) {
      ...
      if (typeof node.textStyleId !== 'symbol') {
        ;(async function () {
          ...
          if (style.key) {
            const newStyleKey = findMatchInSelectedTheme(style.key)
            if (newStyleKey) {
              const newStyle = (await figma.importStyleByKeyAsync(newStyleKey)) as TextStyle
              const fontFamily = newStyle.fontName.family
              const fontStyle = newStyle.fontName.style
...
function applyEffect(node) {
  ...
  if (
    ...
    if (node.effectStyleId) {
      ;(async function () {
        const style = figma.getStyleById(node.effectStyleId) as EffectStyle
        if (style.key) {
          const newStyleKey = findMatchInSelectedTheme(style.key)
          if (newStyleKey) {
            const newStyle = await figma.importStyleByKeyAsync(newStyleKey)
            if (newStyle) {
...
function findMatchInSelectedTheme(styleKey) {
  ...
  const currentStyle = jsonBinData.find((style) => style.key === styleKey)
  ...
  if (currentStyle) {
    ...
    const matchedStyle = jsonBinData.find((style) => style.name === name && style.theme === selectedTheme)
