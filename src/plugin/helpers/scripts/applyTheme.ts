...
export async function applyTheme(themeData, theme) {
  ...
  if (selection.length >= 1) {
    ...
    const actualCount = Object.keys(count).length

    //cancel the current notification
    notify.cancel()

    //add a msg for failed styles
    const numOfFailedStyles = Object.keys(failedStyles).length
    console.log('failed style stuff:', failedStyles, numOfFailedStyles)

    let failedMsg
    if (numOfFailedStyles > 0) {
      const word = numOfFailedStyles > 1 ? ' styles' : ' style'
...
async function applyStyleToNode(node: SceneNode) {
  ...
  if (colorStyles && hasFillStyles(node) && node.fillStyleId != '') {
    ...
    if (typeof node.fillStyleId != 'symbol') {
      ...
      const originalStyle = figma.getStyleById(node.fillStyleId) as PaintStyle

      //see if there is a matching style in the selected theme, apply it if there is
      const matchedStyle = returnMatchingStyle(originalStyle.name, 'PAINT')

      if (matchedStyle !== null) {
        const styleId = await findMatchedKeyOrImportStyle(matchedStyle)
        if (styleId) {
      ...
      const uniqueTextColorStyles = node.getStyledTextSegments(['fillStyleId'])

      for await (const fillStyle of uniqueTextColorStyles) {
        //get the style currently applied to the node
        const originalStyle = figma.getStyleById(fillStyle.fillStyleId)
  ...
  if (colorStyles && hasStrokeStyle(node) && node.strokeStyleId != '') {
    //get the style currently applied to the node
    const originalStyle = figma.getStyleById(node.strokeStyleId) as PaintStyle
  ...
  if (textStyles && node.type === 'TEXT') {
    ...
    if (node.textStyleId != '' && typeof node.textStyleId != 'symbol') {
      ...
      const originalStyle = figma.getStyleById(node.textStyleId) as TextStyle

      //see if there is a matching style in the selected theme
      const matchedStyle = returnMatchingStyle(originalStyle.name, 'TEXT')
      if (matchedStyle !== null) {
        const styleId = await findMatchedKeyOrImportStyle(matchedStyle)
        if (styleId) {
          styles[matchedStyle.key] = styleId
      ...
      const uniqueTextStyles = node.getStyledTextSegments(['textStyleId'])

      for await (const textStyle of uniqueTextStyles) {
        const originalStyle = figma.getStyleById(textStyle.textStyleId)
        const matchedStyle = returnMatchingStyle(originalStyle.name, 'TEXT') as TextStyle
        if (matchedStyle !== null) {
          const styleId = await findMatchedKeyOrImportStyle(matchedStyle)
          if (styleId) {
            styles[matchedStyle.key] = styleId
  ...
  if (effectStyles && hasEffects(node) && node.effectStyleId != '' && typeof node.effectStyleId != 'symbol') {
    ...
    const originalStyle = figma.getStyleById(node.effectStyleId) as EffectStyle

    //see if there is a matching style in the selected theme
    const matchedStyle = returnMatchingStyle(originalStyle.name, 'EFFECT') as EffectStyle
...
function returnMatchingStyle(name, type) {
  ...
  const uniqueThemes = [...new Set(allThemes.map((item) => item.theme.toLowerCase()))]

  //normalize style name for matching
  const normalizedCurrentStyleName = processStyleNameWithThemeNameIncluded(name.toLowerCase(), uniqueThemes)
  ...
  selectedThemeStyles.forEach((style) => {
    const normalizedNewStyleName = processStyleNameWithThemeNameIncluded(style.name.toLowerCase(), uniqueThemes)
...
async function findMatchedKeyOrImportStyle(matchedStyle) {
  ...
  if (styles[matchedStyle.key]) {
    ...
    try {
      ...
      const style = allThemes.find((style) => style.key === matchedStyle.key)
